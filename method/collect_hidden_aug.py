...
def find_contrastive_index(correct_lines, buggy_lines):
    """
...
class HumanEvalPack:
    ...
    def process_single(self, dp, split_token="\n", original_label=1, mutated_label=2):
        ...
        correct_split_list =  utils.find_all_occurrences(split_token, correct_code)
        error_split_list = utils.find_all_occurrences(split_token, buggy_code)
        correct_lines = correct_code.split(split_token)
        buggy_lines = buggy_code.split(split_token)
        result = find_contrastive_index(correct_lines, buggy_lines)
        contrastive_pair = [[], []]
        for item in result:
...
class RepairData:
    def __init__(self,
        ...
        self.processed_data = []
        for item in self.data:
            self.processed_data.append(self.process_single(item, 
                                                           split_token="\n", 
